...
module: service
version_added: 0.1.0
short_description: Manage Harness Service
description:
  - Manage Harness Services.
author:
...
options:
  identifier:
    ...
    type: str
  org:
    description: Identifier of the Harness Organization to which the Service will belong.
    required: True
    type: str
  state:
    description: Desired state of the Harness Service.
    choices:
      - absent
      - present
    default: present
    type: str
  name:
    description: Name of the Harness Service.
    required: False
    type: str
  tags:
    description: A dictionary of tags to add the Harness Service.
    type: dict
  description:
    description: A description to apply to the Harness Service.
    required: False
    type: str
  yaml:
    description: 
...
- name: Create a Harness Service.
  karcadia.harness.service:
    ...
    description: The Service used for a Demo.
    state: present
    tags:
      purpose: demo

- name: Delete a Harness Service.
  karcadia.harness.service:
    identifier: demo_service
    org: my_demo_org
    state: absent
...
RETURN = r"""
service:
  description: The service structure that was created.
  returned: when state is present
  type: dict
  suboptions:
    description:
      description: The description applied to the Harness Service.
      type: str
    identifier:
      description: Identifier of the Harness Service.
      type: str
    name:
      ...
      type: str
    org:
      description: Identifier of the Harness Organization to which the Service belongs.
      type: str
    project:
      description: Identifier of the Harness Project to which the Service belongs.
      type: str
    tags:
      description: A dictionary of tags attached to the Harness Service.
      type: dict
"""
...
def ensure_present(module):
    ...
    yaml        = module.params["yaml"]

    # Ensure all required parameters for a create were provided.
    if not yaml:
      module.fail_json(msg='The yaml parameter must be provided when state is present.')
    ...
    if checked_and_present:
      ...
      existing = loads(harness_response.text)['service']
      needs_update = False
      ...
      if pre_json_object['tags'] and pre_json_object['tags'] != existing['tags']:
        needs_update = True
        component = 'tags'
      if pre_json_object['yaml'] != existing['yaml']:
        needs_update = True
        component = 'yaml'
      ...
      if needs_update:
        ...
        if module.check_mode:
          ...
          module.exit_json(changed=True, msg=f'{module.object_title} {object_id} has been updated.', check_mode=True, service=pre_json_object, updated=True, component_triggering_update=component)
      else:
    ...
    if checked_and_absent:
      if module.check_mode:
        # Return success with the object that we would have created.
        module.exit_json(changed=True, msg=f'{module.object_title} {object_id} has been created.', check_mode=True, service=pre_json_object)
    ...
    if create_object_resp.status_code == 200:
      ...
      module.exit_json(changed=True, msg=f'{module.object_title} {object_id} has been {actioned}.', service=object_resp_dict, updated=True, component_triggering_update=component)
    elif create_object_resp.status_code == 201:
      actioned = 'created'
      # Extract the object returned from the Harness API and return it with our successful module exit.
      object_resp_dict = loads(create_object_resp.text)
      object_resp = object_resp_dict
      module.exit_json(changed=True, msg=f'{module.object_title} {object_id} has been {actioned}.', service=object_resp)
    else:
